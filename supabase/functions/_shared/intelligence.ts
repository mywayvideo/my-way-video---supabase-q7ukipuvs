...
function buildSystemPrompt(context: GenerateContext): string {
  ...
  parts.push(
    ...
    `2. Se o usuário perguntar sobre um produto que NÃO está no catálogo (ausente do array de produtos recebido), você deve informar ao usuário que o item não existe no catálogo, SEM fornecer preço, especificação ou qualquer outra informação sobre ele. EXCEÇÃO: Se a pergunta for de natureza técnica sobre software, ferramentas, fluxos de trabalho, compatibilidade ou tutoriais (ex: "Quais os recursos do vmix?"), OU se a pergunta envolver marcas, fabricantes, equipamentos ou produtos do mercado audiovisual (ex: "snews", "Volks telas interativas para jornalismo"), e existirem dados de Inteligência de Mercado (MI) relevantes no contexto, você DEVE usar esses dados de MI para responder à pergunta, mesmo que não haja um produto físico correspondente no catálogo. Responder com base em dados de MI nessas situações NÃO constitui alucinação — é o uso apropriado e correto da informação de mercado disponível no contexto. A ausência de um item no catálogo de produtos não impede a resposta quando há dados de MI relevantes sobre a marca, fabricante ou equipamento mencionado.\n` +
  ...
  parts.push(
    `CONTEXTO DE INTELIGÊNCIA DE MERCADO (COMPLEMENTAR):\n` +
    `Você pode receber informações de inteligência de mercado (tendências, análises, eventos) como contexto complementar.\n` +
    `Use essas informações para enriquecer suas respostas com insights de mercado relevantes, MAS:\n` +
    `- NUNCA substitua dados do catálogo de produtos por informações de mercado\n` +
    `- NUNCA invente preços, especificações ou disponibilidade baseadas em dados de mercado\n` +
    `- Os dados do catálogo (produtos, preços, estoque) são sempre a fonte primária de verdade\n` +
    `- Mencione tendências ou análises de mercado apenas quando forem diretamente relevantes para a pergunta do usuário\n` +
    `- Se as informações de mercado não forem relevantes, ignore-as silenciosamente`,
  )
