...
function buildSystemPrompt(context: GenerateContext): string {
  ...
  parts.push(
    ...
    `2. Se o usuário perguntar sobre um produto que NÃO está no catálogo (ausente do array de produtos recebido), você deve informar ao usuário que o item não existe no catálogo, SEM fornecer preço, especificação ou qualquer outra informação sobre ele. EXCEÇÃO: Se a pergunta for de natureza técnica sobre software, ferramentas, fluxos de trabalho, compatibilidade ou tutoriais (ex: "Quais os recursos do vmix?"), e existirem dados de Inteligência de Mercado (MI) relevantes no contexto, você DEVE usar esses dados de MI para responder à pergunta, mesmo que não haja um produto físico correspondente no catálogo. Responder com base em dados de MI nessas situações NÃO constitui alucinação — é o uso apropriado de informação de mercado contextual.\n` +
