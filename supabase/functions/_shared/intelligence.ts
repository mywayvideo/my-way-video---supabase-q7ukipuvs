...
function buildSystemPrompt(context: GenerateContext): string {
  ...
  parts.push(
    `REGRA DE GROUNDING (OBRIGATÓRIO E SEVERO — 4 PONTOS):\n` +
      ...
      `2. Se o usuário perguntar sobre um produto que NÃO está no catálogo (ausente do array de produtos recebido), você deve informar ao usuário que o item não existe no catálogo, SEM fornecer preço, especificação ou qualquer outra informação sobre ele.\n` +
      `EXCEÇÃO À REGRA 2: Quando há dados de Inteligência de Mercado (MI) recuperados no contexto, você PODE e DEVE usar esses dados para responder perguntas técnicas sobre software, ferramentas, workflows, compatibilidade, tutoriais e qualquer outro assunto para o qual o contexto de MI foi extraído. Isso NÃO constitui alucinação — é uso legítimo de dados recuperados da base de inteligência de mercado.\n` +
  ...
  parts.push(
    `CONTEXTO DE INTELIGÊNCIA DE MERCADO (OBRIGATÓRIO QUANDO PRESENTE):\n` +
      `Você pode receber informações de inteligência de mercado (tendências, análises, eventos, tutoriais, compatibilidade, workflows) como contexto complementar.\n` +
      `Quando o contexto de MI está presente, você DEVE usá-lo para responder QUALQUER tipo de pergunta a partir da qual o contexto de MI foi extraído — não se limitando a perguntas de compatibilidade, software ou workflows.\n` +
      `Se o fluxo de MI injetou contexto na mensagem, você deve ser capaz de se apoiar nele para responder à pergunta, em vez de afirmar que o item não existe no catálogo.\n` +
      `REGRAS:\n` +
      `- USE os dados de MI para responder perguntas técnicas sobre software, ferramentas, workflows, compatibilidade, tutoriais e qualquer outro tema coberto pelo contexto de MI\n` +
      `- NUNCA invente preços, disponibilidade ou especificações de produtos físicos que não estejam presentes no catálogo\n` +
      `- Os dados do catálogo (produtos físicos, preços, estoque) são sempre a fonte primária de verdade para informações de produtos físicos\n` +
      `- Se as informações de MI não forem relevantes para a pergunta, ignore-as silenciosamente`,
  )
...
function buildMessages(
  ...
  if (context.marketIntelligence && context.marketIntelligence.length > 0) {
    console.log(
      `[MI][6] MI section present in final message: ${context.marketIntelligence.length} items — sending to LLM`,
    )
