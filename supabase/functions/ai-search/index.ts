...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const originalQuery = sanitizeInput(body?.query || '')
    const session_id = typeof body?.session_id === 'string' ? body.session_id : null
    const lastReferencedProductId = body?.currentProductId || null
    const recentProductIds: string[] = Array.isArray(body?.recentProductIds)
      ? body.recentProductIds.filter((id: any) => typeof id === 'string')
      : []
    ...
    if (isHPMode && recentProductIds.length > 0) {
      try {
        const { data: recentProducts, error: recentError } = await supabase
          .from('products')
          .select('id, name, category, manufacturer:manufacturers(name)')
    ...
    const { data: cacheSettingsRow } = await supabase
      .from('cache_settings')
      .select('mi_expiration_days')
      .maybeSingle()
    const miExpirationDays = cacheSettingsRow?.mi_expiration_days ?? 365
    ...
    try {
      ...
      if (!miError && Array.isArray(miRaw)) {
        ...
        for (const mi of marketIntelligenceData) {
          const title = mi.title || '(sem título)'
          const source = mi.source_url ? `(Fonte: ${mi.source_url})` : '(Fonte: N/A)'
          const event = mi.event_name ? `(Evento: ${mi.event_name})` : ''
          console.log(`[ai-search][debug-mi] ${title} ${source} ${event}`)
        }
      }
    } catch (miErr: any) {
    ...
    async function persistAndReturn(aiResult: any, type: string): Promise<Response> {
      ...
      if (typeof result.content === 'string' && Array.isArray(result.products)) {
        ...
        const markerRegex =
          /(?:\[PRODUCT:|<!--\s*PRODUCT(?:_IMAGE)?:\s*)([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:\s*-->|\])/gi
    ...
    if (marketIntelligenceData.length > 0) {
      ...
      aiResult = await generateResponse(query, contextForAI, undefined, supabase)
      const allMiProductIds = miReferencedProducts.map((p: any) => p.id)
      const miReferencedProductData = miReferencedProducts.map((p: any) => ({
        id: p.id,
        name: p.name,
      ...
      return await persistAndReturn(
        {
          ...aiResult,
          referenced_internal_products: allMiProductIds,
          referenced_product_data: miReferencedProductData,
          ai_referenced_count: allMiProductIds.length,
