...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (pricingContext) {
      console.log('[send-order-email] Pricing context:', JSON.stringify(pricingContext))
    }
    if (shippingContext) {
      console.log('[send-order-email] Shipping context:', JSON.stringify(shippingContext))
    }
    ...
    if (resendApiKey) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          console.error(`[send-order-email] 403 Forbidden - Domain verification required for ${fromEmail}. The domain "${fromEmail.split('@')[1]}" may not be verified in Resend. Details: ${errorText}`)
        } else {
